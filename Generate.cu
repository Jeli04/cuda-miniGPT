...
// TransformerBlockCofig struct definition
typedef struct {
    int block_size;
    int n_heads;
    int d_model;
    ...
    // Weight pointers
    float* qkv_weights;
    float* mha_proj_weights;
    float* ln1_weights;
    float* ln2_weights;
    float* ffwd_weights;
    float* lnf_weights;
    float* lm_head_weights;
} TransformerBlockCofig;
...
void generate_tokens_contextual(
    ...
    if (input_length > config.block_size) {
        conditioned_length = config.block_size;
        conditioned_tokens = (int*)malloc(conditioned_length * sizeof(int));

        int start_idx = input_length - config.block_size;
    ...
    debug_conditioning(input_tokens, input_length, conditioned_tokens, conditioned_length, 
                      config.block_size, vocab, vocab_size);

    int max_sequence_length = conditioned_length + max_new_tokens;
    ...
    printf("Starting generation from conditioned text: '%s'\n", full_text);
